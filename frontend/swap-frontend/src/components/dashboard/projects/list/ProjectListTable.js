...
import { Col, Row, Dropdown, Table, Card, ProgressBar, Form, Button } from "react-bootstrap";
  ...
  const columns = useMemo(
    () => [
        ...
        Cell: ({ value, row }) => {
          return (
            <div className="d-flex align-items-center">
              ...
              <ProgressBar variant={getStatusColor(row.original.status)} now={value} className="flex-auto" style={{ height: "6px" }} />
            </div>
          );
        },
      },

      {
        ...
        Header: "STATUS",
        Cell: ({ value }) => {
          return <span className={`badge bg-light-${getStatusColor(value)} text-dark-${getStatusColor(value)}`}>{value}</span>;
        },
      },
      {
        accessor: "team",
          ...
          return (
            <AvatarGroup>
              {ProjectTeamMembersData.filter(function (dataSource) {
                return dataSource.id === value[0] || dataSource.id === value[1] || dataSource.id === value[2];
              }).map((member, index) => {
                return <Avatar size="md" src={member.image} type={`${member.image == null ? "initial" : "image"}`} name={member.name} className="rounded-circle" imgtooltip key={index} />;
              })}
              <Avatar size="md" type="initial" name={value.length - 3 + "+"} variant="light" className="rounded-circle text-dark" showExact />
            </AvatarGroup>
          );
        },
      },
      ...
      <Row className="justify-content-md-between mb-4 mb-xl-0 ">
        <Col xl={2} lg={4} md={6} xs={12}>
          ...
          <div className="mb-2 mb-lg-4">
            <GlobalFilter filter={globalFilter} setFilter={setGlobalFilter} placeholder="Search by project name" />
          </div>
        </Col>
        <Col xxl={2} lg={2} md={6} xs={12}>
          {/* records filtering options */}
          <Form.Control as={FormSelect} placeholder="Filter" options={categoryOptions} />
        </Col>
      </Row>
                  ...
                  <tbody {...getTableBodyProps()}>
                    ...
                    <tr>
                      <td className="align-middle " colSpan="7">
                        <div className="d-flex align-items-center">
                          <Button variant="link" className="text-muted border border-2 rounded-3 card-dashed-hover p-0" onClick={onNewProject}>
