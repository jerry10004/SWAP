...
const ProjectGrid = () => {
  ...
  const [ProjectsList, setProjectsList] = useState(ProjectsGridData.slice(0, 500));
  ...
  const displayProjects = ProjectsList.slice(pagesVisited, pagesVisited + projectsPerPage).map((item, index) => {
    return (
  ...
  const getSearchTerm = (event) => {
    ...
    if (searchTerm !== "") {
      const newProjectsList = ProjectsGridData.filter((project) => {
        return Object.values(project).join(" ").toLowerCase().includes(searchTerm.toLowerCase());
      });
        ...
        <Col lg={12} md={12} sm={12}>
          <div className="border-bottom pb-4 mb-4 d-md-flex align-items-center justify-content-between">
            ...
            <div>
              ...
              <Offcanvas show={show} onHide={handleClose} placement="end" name="end" style={{ width: "600px" }}>
      ...
      <Row className="justify-content-md-between mb-4 mb-xl-0 ">
        <Col xl={2} lg={4} md={6} xs={12}>
          ...
          <div className="mb-2 mb-lg-4">
            <Form.Control type="search" placeholder="Search by project name" value={searchTerm} onChange={getSearchTerm} />
          </div>
        </Col>
        <Col xxl={2} lg={2} md={6} xs={12}>
          {/* records filtering options */}
          <Form.Control as={FormSelect} placeholder="Filter" options={categoryOptions} onChange={getFilterTerm} />
        </Col>
      </Row>

      {/* displaying records */}
      <Row>{displayProjects.length > 0 ? displayProjects : <Col>No matching projects found.</Col>}</Row>
    </Fragment>
  );
};
