...
const StudentsGridCard = () => {
  ...
  const displayStudents = students.slice(pagesVisited, pagesVisited + studentsPerPage).map((students) => {
    return (
          ...
          <Card.Body>
            <div className="text-center">
              <div className="position-relative">
                <Image src={students.image} className="rounded-circle avatar-xl mb-3" alt="" />
              </div>
            ...
            <div className="d-flex justify-content-between border-bottom py-2 mt-6">
              <span>Payments</span>
              <span className="text-dark">${numberWithCommas(students.payment)}</span>
            </div>
  ...
  const getSearchTerm = (event) => {
    ...
    if (searchTerm !== "") {
      const newStudentsList = StudentsList.filter((student) => {
        return Object.values(student).join(" ").toLowerCase().includes(searchTerm.toLowerCase());
      });
  ...
  return (
    <Fragment>
      <div className="mb-4">
        <Form.Control type="search" placeholder="Search Students" value={searchTerm} onChange={getSearchTerm} />
      </div>
      <Row>{displayStudents.length > 0 ? displayStudents : <Col>No matching students found.</Col>}</Row>
