...
function InstructorsGridCard() {
  ...
  const displayInstructors = instructors.slice(pagesVisited, pagesVisited + instructorsPerPage).map((instructors) => {
    return (
      <Col xl={3} lg={6} md={6} sm={12} key={instructors.id}>
        <Card className="mb-5">
          <Card.Body>
            <div className="text-center">
              <Image src={instructors.image} className="rounded-circle avatar-xl mb-3" alt="" />
            ...
            <div className="d-flex justify-content-between border-bottom py-2 mt-4">
              <span>Students</span>
              <span className="text-dark">{numberWithCommas(instructors.students)}</span>
            </div>
            <div className="d-flex justify-content-between border-bottom py-2">
              <span>Instructor Rating</span>
              <span className="text-warning">
                {instructors.rating} <Icon path={mdiStar} size={0.6} className="mb-1" />{" "}
              </span>
            </div>
  ...
  const getSearchTerm = (event) => {
    ...
    if (searchTerm !== "") {
      const newInstructorsList = InstructorData.filter((instructor) => {
        return Object.values(instructor).join(" ").toLowerCase().includes(searchTerm.toLowerCase());
      });
  ...
  return (
    <Fragment>
      <div className="mb-4">
        <Form.Control type="search" placeholder="Search Instructors" value={searchTerm} onChange={getSearchTerm} />
      </div>
      <Row>{displayInstructors.length > 0 ? displayInstructors : <Col>No matching instructors found.</Col>}</Row>
